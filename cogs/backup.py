...
async def request_code(ctx, title = "⌨ | Пожалуйста, введите код", code = None, reifnone = True):
    ...
    if reifnone:
        ...
        code = "0" * 10
    emb = disnake.Embed(title = title, description = "```\n```", color = Color.primary)
    emb.add_field(name = "Забыли код?", value = "~~Обратитесь в [поддержку](https://discord.gg/VtJuw3P7qE)~~ Смиритесь с этим.")
    ...
    while len(entered) != len(code):
        inter = await bot.wait_for('button_click',check=check)
        if inter.component.label.isdigit():
            entered += inter.component.label
            emb.description = f"```\n{'•' * len(entered)}\n```"
            ...
            if not a[1]:
                if a[0]:
                    ...
                    while next:
                        ...
                        elif inter.component.label == "Нет":
            ...
            if not a[1]:
                if a[0]:
                    ...
                    while next:
                        ...
                        if inter.author == ctx.author:
                            next = False
                        if inter.component.label == "Полное восстановление":
                            await inter.response.defer()
                            if ctx.author == ctx.guild.owner:
                                data = mongo.db.backups.find_one({"_id": ctx.guild.id})
                                ...
                                for k in range(250):
                                    ...
                                    try:
                                        await ctx.guild.create_role(
                                            ...
                                            colour=disnake.Colour(value=data['roles'][str(j)]['color']),
                                            permissions=disnake.Permissions(permissions=data['roles'][str(j)]['perms']),
