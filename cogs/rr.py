...
class RR(commands.Cog):
    ...
    async def addsingle(self, ctx):
        ...
        embed = disnake.Embed(title="🚩 | Роли за реакции", color=Color.primary, description="Пожалуйста, через пробел укажите роли, которые будут выдаваться при добавлении реакции.")
...
**Сообщение:** [Перейти]({r[0].message.jump_url})
    ...
    async def delsingle(self, ctx):
        embed = disnake.Embed(title="🚩 | Роли за реакции", color=Color.primary, description="Пожалуйста, поставьте реакцию, которую нужно убрать.")
        msg = await ctx.send(embed=embed)
        def reaction_check(reaction, user):
            return reaction.message.guild.id == ctx.guild.id and user == ctx.author
