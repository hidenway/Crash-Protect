...
class AntiSpam(commands.Cog):
    ...
    async def antiinvite(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            embed = disnake.Embed(title = "🔗 | Анти-ссылки-приглашения")
...
**Состояние:** {enabled}
    ...
    async def ic(self, ctx, channels: commands.Greedy[disnake.TextChannel]):
        with ctx.channel.typing():
    ...
    async def ir(self, ctx, roles: commands.Greedy[disnake.Role]):
        with ctx.channel.typing():
            if not ctx.guild.id in cache.antiinvite_data:
                cache.antiinvite_data[ctx.guild.id] = {}
            try:
    ...
    async def antiflood(self, ctx):
        measures.add(what=7)
        if ctx.invoked_subcommand is None:
            embed = disnake.Embed(title = "💥 | Анти-флуд")
            embed.color = Color.primary
            p = ctx.prefix
            embed.description = f'''
`<обязательный параметр>` `[необязательный параметр]`
...
**Максимум сообщений:** {maxmsg}
    ...
    async def _ic(self, ctx, channels: commands.Greedy[disnake.TextChannel]):
        with ctx.channel.typing():
    ...
    async def _ir(self, ctx, roles: commands.Greedy[disnake.Role]):
        with ctx.channel.typing():
            if not ctx.guild.id in cache.antiflood_data:
                cache.antiflood_data[ctx.guild.id] = {}
            try:
