...
async def send_graph(
    ...
    await ctx.send(file=disnake.File(f"{code}.png"))
    os.remove(f"{code}.png")
...
async def err(ctx, message, reset_cooldown=False):
    ...
    embed.set_footer(icon_url=ctx.author.display_avatar, text=ctx.author)
...
def generate_progressbar(amount):
    ...
    bar = "<:progress_begin_unfill:1071732053806153749>"
    if amount >= 10:
        bar = "<:progress_begin_fill:1071732414340157441>"
    bar += "<:progress_middle_fill:1071732951731146812>" * (amount//10)
    bar += "<:progress_middle_unfill:1071733317805817886>" * (9-(amount//10))
    if amount >= 100:
        bar += "<:progress_end_fill:1071734821753208882>"
    else:
        bar += "<:progress_end_unfill:1071735083926552657>"
    return bar
